    ...
    >
      <div
        ...
        onPointerDown={(e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          onHandlePointerDown(bill.id, e);
        }}
        title={canDrag ? 'Tap and drag to reorder' : 'Paid items stay at bottom'}
      >
        <GripVertical size={18} />
      </div>

      <div className="bill-name" title={bill.title}>{bill.title}</div>
...
export default function App() {
  ...
  const sortedBills = useMemo(() => {
    ...
    if (!sortType) 
const unpaid = items.filter(b => getDerivedStatus(b) !== 'Paid');
const paid = items.filter(b => getDerivedStatus(b) === 'Paid');
return [...unpaid, ...paid];
    ...
    items.sort((a, b) => {
      ...
      if (sortType === 'name') {
        ...
        valA = statusRank[getDerivedStatus(a)] ?? 999;
        valB = statusRank[getDerivedStatus(b)] ?? 999;
      } else {
        return 0;
      }
...
return [...unpaid, ...paid];
  ...
  useEffect(() => {
    ...
    function handlePointerMove(e) {
      ...
      const row = el?.closest?.('[data-bill-id]');
      if (!row) return;

      const canDrag = row.getAttribute('data-can-drag') === '1';
      if (!canDrag) return;

      const id = Number(row.getAttribute('data-bill-id'));
      if (!Number.isNaN(id) && id !== dragOverId) {
        setDragOverId(id);
      }
    }

    function handlePointerUp() {
      handleDrop(dragOverId || draggedId);
    }
        ...
        <div className="top-section">
          ...
          <div className="sort-row">
            <button
              ...
              type="button"
              onClick={() => {
                const nextOrder = sortType === 'name' && sortOrder === 'asc' ? 'desc' : 'asc';
                setSortType('name');
...
return [...unpaid, ...paid];
            ...
            <button
              className={`sort-btn ${sortType === 'amount' ? 'active' : ''}`}
              type="button"
              onClick={() => {
                const nextOrder = sortType === 'amount' && sortOrder === 'asc' ? 'desc' : 'asc';
                setSortType('amount');
                setSortOrder(nextOrder);
                setBills((prev) => {
                  const items = [...prev];
                  items.sort((a, b) => {
                    if (a.amount < b.amount) return nextOrder === 'asc' ? -1 : 1;
                    if (a.amount > b.amount) return nextOrder === 'asc' ? 1 : -1;
                    return 0;
                  });
...
return [...unpaid, ...paid];
            ...
            <button
              className={`sort-btn ${sortType === 'status' ? 'active' : ''}`}
              type="button"
              onClick={() => {
                const nextOrder = sortType === 'status' && sortOrder === 'asc' ? 'desc' : 'asc';
                const rank = { Late: 1, Due: 2, Unpaid: 3, Paid: 4 };
                setSortType('status');
                setSortOrder(nextOrder);
                setBills((prev) => {
                  const items = [...prev];
                  items.sort((a, b) => {
                    const valA = rank[getDerivedStatus(a)] ?? 999;
                    const valB = rank[getDerivedStatus(b)] ?? 999;
                    if (valA < valB) return nextOrder === 'asc' ? -1 : 1;
                    if (valA > valB) return nextOrder === 'asc' ? 1 : -1;
                    return 0;
                  });
...
return [...unpaid, ...paid];
            ...
            >
              Status {sortType === 'status' ? (sortOrder === 'asc' ? '↑' : '↓') : ''}
            </button>
          </div>
          ...
          <div className="list-card">
            {bills.map((bill) => (
              <BillRow
                ...
                onHandlePointerDown={handleDragStart}
